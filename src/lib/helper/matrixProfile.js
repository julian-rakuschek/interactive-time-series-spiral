// @ts-nocheck
/*
SOURCE: https://github.com/hitzelc/matrixprofile_js/blob/master/matrixprofile.js
Author: Chris Hitzel
Published: May 21, 2020
Copied on: August 29, 2025
License: MIT
...
function apply_exclusion_zone(exclusion_zone, is_join, window_size, data_length, index, distance_profile){
    if(exclusion_zone > 0 && !is_join){
        let ez_start = Math.max(0, index - exclusion_zone);
        let ez_end = Math.min(data_length - window_size + 1, index + exclusion_zone);
...
export function MASS(query,timeseries,runnum){
...
export function STAMP(t_a,t_b,subseq_length){
